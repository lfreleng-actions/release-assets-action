...
- **Glob patterns** - Upload multiple files with wildcards
...
A JSON array of file paths or glob patterns. The action will expand globs and upload all
matching files.
...
The git tag associated with the release. If not provided, the action automatically extracts
it from `github.ref` when the workflow is triggered by a tag push.

**Note:** The release must already exist. This action does not create releases.

#### `release_name`

Optional release name for additional validation. If provided along with `release_tag`,
both must match the target release, otherwise the action fails.
...
This action uses `actions/github-script` to execute JavaScript directly within the workflow,
avoiding the need for compiled artifacts or separate source files. The implementation:

1. **Parses inputs** - Validates JSON array format for asset paths
2. **Determines target** - Gets release tag from input or GitHub context
3. **Finds release** - Uses GitHub API to locate the release by tag (and optionally name)
...
The action works with both draft and published releases, as long as they exist in the
repository.
...
If you specify both `release_tag` and `release_name`, they must both match the target
release. Check that the release name in GitHub matches exactly.
...
- The action does **not** create releases - it only uploads to existing ones
- Glob patterns follow standard glob syntax (uses Node.js `glob` package)
- Multiple files with the same name will overwrite each other
