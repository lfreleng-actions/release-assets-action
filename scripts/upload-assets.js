 ...
 * @param {boolean|string} params.denyOverwrite - Prevent overwriting existing assets (optional, default: 'true')
 */
module.exports = async ({ github, context, core, glob, assetPathsInput, releaseTag, releaseName, denyOverwrite }) => {
  ...
  // Note: trim() is safe here - GitHub Actions YAML interpolation guarantees these are strings
  // (defaults to '' from action.yaml, never null/undefined), so no null checks needed
  ...
  if (!release && (targetTag || trimmedReleaseName)) {
    ...
    const MAX_PAGES = 100; // Search up to 10,000 releases
    let page = 1;

    while (!release && page <= MAX_PAGES) {
    ...
    if (!release && page > MAX_PAGES) {
      core.warning(`Stopped searching after ${MAX_PAGES} pages (10,000 releases). Release may exist but was not found.`);
    }
  ...
  for (const pattern of assetPaths) {
    ...
    // Note: statSync is appropriate here - this is a single-task CI script
    // with no concurrent operations, so event loop blocking is not a concern
    const files = [];
  ...
  // Check for basename collisions (files from different directories with same name)
  ...
  for (const filePath of uniqueFilesToUpload) {
    ...
    if (baseNameMap.has(baseName)) {
      collisions.push({
        name: baseName,
        paths: [baseNameMap.get(baseName), filePath]
      });
      // Add subsequent collisions to the same entry
      const existingCollision = collisions.find(c => c.name === baseName);
      if (existingCollision && !existingCollision.paths.includes(filePath)) {
        existingCollision.paths.push(filePath);
      }
    } else {
  ...
  if (collisions.length > 0) {
    let errorMessage = 'Basename collision detected: Multiple files with the same name from different directories:\n';
    for (const collision of collisions) {
      errorMessage += `\n  "${collision.name}" from:\n`;
      for (const p of collision.paths) {
        errorMessage += `    - ${p}\n`;
      }
    }
    errorMessage += '\nGitHub releases only support one file per name. Please ensure file names are unique.';
  ...
  // Upload each file
  // Note: Files are uploaded sequentially, not in parallel. This is intentional because:
  // 1. Most releases have < 10 assets (performance impact is minimal)
  // 2. Sequential uploads are more predictable and easier to debug
  // 3. If an upload fails, the action stops immediately (fail-fast behavior)
  // 4. Partial uploads can occur on failure - re-run the action to complete
  const downloadUrls = [];
  for (const filePath of uniqueFilesToUpload) {
    const fileName = path.basename(filePath);
    // Note: Using readFileSync (buffer) instead of streaming because:
    // 1. Release assets are typically < 500MB (GitHub limit is 2GB)
    // 2. Actions runners have 7-14GB RAM available
    // 3. Buffers allow simple retry logic (streams can't be reused)
    // 4. Sequential uploads mean no concurrent memory pressure
    const fileContent = fs.readFileSync(filePath);

    core.info(`Uploading: ${fileName} (${fileContent.length} bytes)`);

    // Check if asset already exists (proactive duplicate detection)
    // Note: Defensive check for release.assets - while GitHub API should always return
    // this array, we handle undefined/null to prevent crashes in edge cases
    const existingAsset = (release.assets || []).find(a => a.name === fileName);
    if (existingAsset) {
      if (denyOverwriteBool) {
        core.setFailed(
          `Error: Asset '${fileName}' already exists in this release ❌\n` +
          `Overwriting is not allowed (deny_overwrite=true).\n` +
      ...
      core.warning(`Asset '${fileName}' already exists - will be replaced`);
    ...
    try {
      ...
      core.error(`Note: ${downloadUrls.length} file(s) were already uploaded before this failure.`);
      throw error;
    }
  }
